...
if platform == "msvc" then
    function cc( target )
        ...
        if target.settings.optimization then
            ccflags = ccflags.." /GF /O2 /Ob2 /Oi /Ot /Oy /Gs /GS-";
        end

        if target.settings.fast_floating_point then
            ccflags = ccflags.." /fp:fast";
        end

        if target.settings.sse2 then
            ccflags = ccflags.." /arch:SSE2";
        end
    ...
    function build_executable( target )
        ...
        elseif target:prototype() == DynamicLibraryPrototype then
        ...
        if objects ~= "" then
            ...
            if target.settings.incremental_linking then
                ...
                if exists(embedded_manifest) ~= true then        
                    ...
                    system( msrc, "rc /Fo\""..embedded_manifest_res.."\" "..embedded_manifest_rc, IgnoreOutputScanner );
                end
