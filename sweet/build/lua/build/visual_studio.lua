...
local function configurations( vcproj, target, include_directories )
    ...
    for _, platform in ipairs(target.settings.platforms) do
        for variant, variant_settings in pairs(target.settings.settings_by_variant) do
            local output = "";
            ...
            <Configuration
                ...
                <Tool
                    Name="VCNMakeTool"
                    BuildCommandLine="build command=build variant=]]..variant..[[ platform=]]..platform..[[ goal=]]..target:id()..[["
                    ReBuildCommandLine="build command=clean variant=]]..variant..[[ platform=]]..platform..[[ goal=]]..target:id()..[[ &#x0D;&#x0A; build command=build variant=]]..variant..[[ platform=]]..platform..[[ goal=]]..target:id()..[["
                    CleanCommandLine="build command=clean variant=]]..variant..[[ platform=]]..platform..[[ goal=]]..target:id()..[["
                    Output="]]..output..[["
...
# Visual Studio 2008
    ...
    for target in solution:get_dependencies() do
        if target:prototype() == ProjectPrototype then
